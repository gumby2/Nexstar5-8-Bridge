...
static const unsigned long POST_GOTO_POLL_RESUME_DELAY_MS = 15000;
...
bool enqueueGotoRaDec(double raDeg, double decDeg, uint8_t source, const char* reason) {
  if (hasQueuedGoto()) {
    gotoQueueReplaced++;
    LOG_MOUNT_W("Rejecting duplicate RA/Dec GOTO while queued %s GOTO is waiting; queued age=%lu ms",
                queuedGotoTypeName(queuedGotoType),
...
bool enqueueGotoAltAz(double altDeg, double azDeg, uint8_t source, const char* reason) {
  if (hasQueuedGoto()) {
    gotoQueueReplaced++;
    LOG_MOUNT_W("Rejecting duplicate Alt/Az GOTO while queued %s GOTO is waiting; queued age=%lu ms",
                queuedGotoTypeName(queuedGotoType),
                (unsigned long)(millis() - queuedGotoMs));
    return false;
  }
...
void serviceGotoCompletionWatch() {
  PROFILE_SCOPE(PROFILE_GOTO_COMPLETION);
  ...
  if (gotoCompletionWatchActive) {
    ...
    while (mountRawAvailable()) {
      ...
      if (b == '@') {
        LOG_MOUNT_I("SkySafari GOTO completion '@' received; clearing LX200 UI slew state without blocking post-GOTO verification");

        // Do not let delayed mount bytes become the first byte of the next E/Z
        // poll.  The completion watcher remains active while this bounded drain
        // observes a quiet interval, so normal polling cannot restart early.
        drainMountAfterCompletion();

        // The original NexStar is now done with the GOTO.  Do not run a slower
        // post-GOTO E/Z verification before telling app clients that the slew is
        // complete.  SkySafari may report "telescope not responding" if GR/GD/D
        // replies are delayed while the firmware waits on mount serial reads.
        ...
        // Normal mount polling will overwrite this estimate after the post-GOTO
        // quiet window.  Do not run a blocking E/Z verification here because
        // SkySafari treats that response gap as "telescope not responding".
        primeGotoCompletionRaDec(gotoCompletionTargetRA_deg, gotoCompletionTargetDec_deg, nowMs);
        LOG_MOUNT_I("SkySafari app-facing RA/Dec cache primed at GOTO completion: RA=%.6f Dec=%.6f until normal polling refreshes",
        ...
        asyncSlewRunning = false;
        deferMountPolling(POST_GOTO_POLL_RESUME_DELAY_MS,
                          "GOTO '@' received; waiting before mount polling resumes");
        return;
      }
    ...
    LOG_MOUNT_I("SkySafari GOTO still waiting for NexStar final '@'; elapsed=%lu ms",
                  (unsigned long)(nowMs - gotoCompletionWatchStartMs));
    }

    if (nowMs - gotoCompletionWatchStartMs > GOTO_TIMEOUT_MS) {
      LOG_MOUNT_W("SkySafari GOTO timed out waiting for final '@'; not sending RA/Dec verification because mount completion is unknown");
      gotoCompletionWatchActive = false;
      gotoCompletionVerifyPending = false;
      asyncSlewRunning = false;
  ...
  if (gotoCompletionVerifyPending) {
    gotoCompletionVerifyPending = false;
    gotoCompletionVerifyDueMs = 0;
    LOG_MOUNT_W("Discarded obsolete post-GOTO verification request; normal polling will refresh position");
  }
}

void serviceAsyncSlew() {
  PROFILE_SCOPE(PROFILE_ASYNC_SLEW);
  ...
  if (asyncSlewPending) {
    ...
    if (gotoRaDec(pendingRA_deg, pendingDec_deg)) {
      ...
      LOG_MOUNT_I("SkySafari GOTO payload sent; watching non-blocking for NexStar final '@'");
    } else {
      asyncSlewRunning = false;
