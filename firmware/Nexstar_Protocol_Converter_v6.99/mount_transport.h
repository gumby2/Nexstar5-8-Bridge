...
uint16_t drainMountAfterCompletion(unsigned long maxMs = 250UL, unsigned long quietMs = 50UL);

void runRawMountDiagnostic(Print &out, uint16_t cycles, bool includeAltAz, unsigned long intervalMs);
void runRawGotoSelfDiagnostic(Print &out, uint16_t cycles, bool waitForCompletion, unsigned long intervalMs);
