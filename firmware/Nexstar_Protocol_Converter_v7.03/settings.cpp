...
bool loadSettingsNVS() {
  ...
  if (mountHandshakeTimeoutMs < 500UL || mountHandshakeTimeoutMs > 10000UL) mountHandshakeTimeoutMs = 3000UL;
...
bool loadPersistentSettings() {
  ...
  if (mountHandshakeTimeoutMs < 500UL || mountHandshakeTimeoutMs > 10000UL) mountHandshakeTimeoutMs = 3000;
...
bool loadFirmwareLoggingEnabled() {
  ...
  firmwareLoggingEnabled = prefs.getBool("fwLogging", true);
  prefs.end();
  return true;
#else
...
bool saveFirmwareLoggingEnabled(bool enabled) {
  firmwareLoggingEnabled = enabled;
#if defined(ESP32)
  Preferences prefs;
  if (!prefs.begin("nexstar", false)) return false;
  prefs.putBool("fwLogging", firmwareLoggingEnabled);
  prefs.end();
#endif
  return true;
}
