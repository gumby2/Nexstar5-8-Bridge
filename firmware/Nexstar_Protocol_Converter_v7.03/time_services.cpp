...
bool syncTimeFromNTP(bool forceLog) {
  ...
  struct tm tminfo;
  if (forceLog) LOG_TIME_T("NTP getLocalTime waiting up to 1500 ms");
  bool ok = getLocalTime(&tminfo, 1500);
  if (!ok) {
    ntpSyncValid = false;
...
void serviceNtpSync() {
  ...
  if (timeValid && currentTimeSource == TIME_SRC_SKYSAFARI) {
    // SkySafari supplies the active observing time/location. Do not let a
    // background NTP lookup stall the LX200 socket immediately after connect.
