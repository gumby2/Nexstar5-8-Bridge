...
extern bool alpacaClientActivitySeen();
...
extern unsigned long httpSkippedForSkySafari;
extern unsigned long httpServicedCount;
...
String basicSystemHealthText() {
  String s;
  s += "SCHEDULER\n";
  s += "Active mount poll interval: " + String(pollIntervalMs) + " ms\n";
...
String systemHealthText() {
  ...
  s += "Active interval: " + String(pollIntervalMs) + " ms\n";
  s += "Idle interval: " + String(idlePollIntervalMs) + " ms\n";
  s += "Effective interval: " + String(effectiveMountPollIntervalMs()) + " ms\n";
  s += "Handshake timeout: " + String(mountHandshakeTimeoutMs) + " ms\n";
  ...
  s += "Free heap: " + String(ESP.getFreeHeap()) + " bytes\n";
#if defined(ESP32)
  s += "Minimum free heap: " + String(espMinFreeHeapCompat()) + " bytes\n";
#endif
  ...
  s += "HTTP serviced/skipped for SkySafari: " + String(httpServicedCount) + "/" + String(httpSkippedForSkySafari) + "\n\n";
...
const char *sampleBannerSystemText() {
  ...
  const char *cpuTextValue = cpuText.c_str();
  if (strncmp(cpuTextValue, "CPU Load ", 9) == 0) cpuTextValue += 9;
  ...
  snprintf(row, sizeof(row), "CPU:%s Loop:%s I0:%s I1:%s",
           cpuTextValue, loopPct, idle0Pct, idle1Pct);
  static char result[72];
  strncpy(result, row, sizeof(result) - 1);
...
void telnetDrawMonitor(Print &out) {
  ...
  out.printf("%-12s %-12s %-12s %-12s %-12lu %-12lu\n",
             ...
             mountPollsSucceeded);
  out.println();
  out.printf("%-12s %-12s %-12s %-12s\n", "ALT", "AZ", "MOUNTPOLL", "LEGACY IDLE");
  out.printf("%-12.6f %-12.6f %-12s %-12s\n",
             mountCurrentAlt_deg,
             mountCurrentAz_deg,
  ...
  out.printf("%-12s %-12s %-12s %-12lu %-12s\n",
             "Alpaca",
             alpacaClientActivitySeen() ? "active" : "idle",
             "-",
             alpacaHttpRequests,
             "-");
  out.println();
