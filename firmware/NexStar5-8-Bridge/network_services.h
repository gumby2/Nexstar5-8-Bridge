...
#else
extern AsyncWebCompatServer *webServer;
extern AsyncWebCompatServer *alpacaServer;
...
extern unsigned long httpLastServiceMs;
...
extern bool telnetRedTextEnabled;
...
extern bool telnetMenuActive;
void telnetStartMenu(Print &out);
void telnetApplyTextColor(Print &out);
void telnetSetMenuTerminalSize(uint16_t columns, uint16_t rows);
uint16_t telnetGetMenuTerminalColumns();
uint16_t telnetGetMenuTerminalRows();
bool telnetMenuSizeFromNaws();
void telnetStopMenu(Print &out);
