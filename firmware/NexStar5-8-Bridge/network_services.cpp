...
#else
AsyncWebCompatServer *webServer = nullptr;
AsyncWebCompatServer *alpacaServer = nullptr;
thread_local AsyncWebCompatServer *activeServer = nullptr;
#endif
...
unsigned long httpSkippedForSkySafari = 0;
unsigned long httpServicedCount = 0;
unsigned long httpLastServiceMs = 0;
static const unsigned long HTTP_SERVICE_MIN_GAP_MS = 250;
static const unsigned long SKYSAFARI_HTTP_QUIET_MS = 10000;
static const unsigned long SKYSAFARI_HTTP_MAX_SKIP_MS = 1500;
static const unsigned long LX200_STALE_CLIENT_MS = 30000;
...
static void acceptLX200Client(WiFiClient &newClient, const char *context) {
  if (lx200Client) lx200Client.stop();
  lx200Client = newClient;
  lx200Client.setNoDelay(true);
  lx200WifiClientConnected = true;
  lx200WifiLastRxMs = millis();
  lx200WifiLastTxMs = 0;
  LOGI("LX200/SkySafari WiFi client %s from %s:%u",
       context,
       lx200Client.remoteIP().toString().c_str(),
       lx200Client.remotePort());
}

static unsigned long lx200ClientLastActivityMs() {
  unsigned long lastMs = 0;
  if (lx200WifiLastRxMs) lastMs = lx200WifiLastRxMs;
  if (lx200WifiLastTxMs && (!lastMs || (long)(lx200WifiLastTxMs - lastMs) > 0)) lastMs = lx200WifiLastTxMs;
  return lastMs;
}

static bool lx200ActiveClientIsStale(unsigned long nowMs) {
  if (!lx200Client || !lx200Client.connected()) return false;
  if (lx200Client.available() > 0) return false;
  unsigned long lastMs = lx200ClientLastActivityMs();
  if (!lastMs) lastMs = nowMs;
  return (nowMs - lastMs) >= LX200_STALE_CLIENT_MS;
}

static bool acceptLX200ClientIfPending(const char *context) {
  if (!lx200ServerPtr) return false;
  WiFiClient newClient = lx200ServerPtr->available();
  if (!newClient) return false;

  if (lx200Client && lx200Client.connected()) {
    unsigned long nowMs = millis();
    if (!lx200ActiveClientIsStale(nowMs)) {
      LOG_SKY_D("LX200/SkySafari pending %s client ignored while active client is still connected",
                context);
      newClient.stop();
      return false;
    }
    unsigned long ageMs = nowMs - lx200ClientLastActivityMs();
    LOG_SKY_W("Replacing stale LX200/SkySafari WiFi client idle for %lu ms with new %s client",
              (unsigned long)ageMs,
              context);
  }
  acceptLX200Client(newClient, context);
  return true;
}

static void dropStaleLX200ClientIfNeeded() {
  if (!lx200ActiveClientIsStale(millis())) return;
  unsigned long ageMs = millis() - lx200ClientLastActivityMs();
  LOG_SKY_W("Dropping stale LX200/SkySafari WiFi client idle for %lu ms",
            (unsigned long)ageMs);
  lx200Client.stop();
...
static const uint8_t TELNET_HISTORY_LINES = 6;
static String telnetHistory[TELNET_HISTORY_LINES];
static uint8_t telnetHistoryCount = 0;
static uint8_t telnetHistoryWrite = 0;
static int8_t telnetHistoryBrowse = -1;
static String telnetHistoryScratch = "";
static uint8_t telnetCommandEscapeState = 0;
static size_t telnetRenderedLineLength = 0;
...
static const size_t TELNET_MENU_PAGE_BUFFER_SIZE = 1536;
...
static uint32_t telnetMenuHeapBeforeFree = 0;
static uint32_t telnetMenuHeapBeforeLargest = 0;
static uint32_t telnetMenuHeapAfterFree = 0;
static uint32_t telnetMenuHeapAfterLargest = 0;
static bool telnetMenuHeapDiagPending = false;
...
// network stack after a link loss is not. Keep reconnect state small and run it
// from the normal loop with bounded work and exponential backoff.
static bool wifiReconnectInitialized = false;
static bool wifiReconnectWasConnected = false;
static bool wifiReconnectAttemptActive = false;
...
static const uint32_t WIFI_RECONNECT_MIN_HEAP = 24000U;
static const uint32_t NETWORK_LOAD_SHED_HEAP = 12000U;
#endif
...
extern const char *sampleBannerSystemText();
...
static bool telnetMenuHeapAllowed(Print &out) {
#if defined(ESP32)
  const uint32_t freeHeap = ESP.getFreeHeap();
  const uint32_t largestBlock = heap_caps_get_largest_free_block(MALLOC_CAP_8BIT);
  const bool btMode = (bridgeMode == BRIDGE_MODE_BT_MIN_WEB);
  const bool lowHeap = (freeHeap < 12000U || largestBlock < 4096U);
  if (btMode && lowHeap) {
    Serial.printf("[TELNET_DIAG] menu blocked in BT mode: free=%u largest=%u min=%u\n",
                  (unsigned)freeHeap,
                  (unsigned)largestBlock,
                  (unsigned)espMinFreeHeapCompat());
    out.println("Full-screen Telnet menu disabled in BT mode: heap margin is too low.");
    out.printf("Heap: free=%u largest=%u minimum=%u bytes\r\n",
               (unsigned)freeHeap,
               (unsigned)largestBlock,
               (unsigned)espMinFreeHeapCompat());
    out.println("Use plain commands instead: help, status, get, mountpoll, mode, reboot.");
    return false;
  }
  if (lowHeap) {
    Serial.printf("[TELNET_DIAG] menu blocked: free=%u largest=%u min=%u\n",
                  (unsigned)freeHeap,
                  (unsigned)largestBlock,
                  (unsigned)espMinFreeHeapCompat());
    out.println("Full-screen Telnet menu disabled: heap margin is too low.");
    out.printf("Heap: free=%u largest=%u minimum=%u bytes\r\n",
               (unsigned)freeHeap,
               (unsigned)largestBlock,
               (unsigned)espMinFreeHeapCompat());
...
static void telnetMenuHeapDiag(const char *stage) {
#if defined(ESP32)
  const uint32_t freeHeap = ESP.getFreeHeap();
  const uint32_t largestBlock = heap_caps_get_largest_free_block(MALLOC_CAP_8BIT);
  if (stage && strcmp(stage, "menu-before") == 0) {
    telnetMenuHeapBeforeFree = freeHeap;
    telnetMenuHeapBeforeLargest = largestBlock;
  } else if (stage && strcmp(stage, "menu-after") == 0) {
    telnetMenuHeapAfterFree = freeHeap;
    telnetMenuHeapAfterLargest = largestBlock;
    telnetMenuHeapDiagPending = true;
  }
#else
  (void)stage;
#endif
...
static void telnetMenuHeapDiagClosed() {
#if defined(ESP32)
  if (!telnetMenuHeapDiagPending) return;
  Serial.printf("[MENU_HEAP] before=%u/%u after=%u/%u closed=%u/%u\n",
                (unsigned)telnetMenuHeapBeforeFree,
                (unsigned)telnetMenuHeapBeforeLargest,
                (unsigned)telnetMenuHeapAfterFree,
                (unsigned)telnetMenuHeapAfterLargest,
                (unsigned)ESP.getFreeHeap(),
                (unsigned)heap_caps_get_largest_free_block(MALLOC_CAP_8BIT));
  telnetMenuHeapDiagPending = false;
#endif
}
...
public:
  ...
  size_t write(const uint8_t *data, size_t size) override {
    if (stalled_ || !data || !size || !client_.connected()) {
      stalled_ = true;
      return 0;
    }
#if defined(ESP32)
...
void serviceHttpServers() {
  PROFILE_SCOPE(PROFILE_HTTP_SERVERS);
  if (!bridgeModeHasWebUi() && !alpacaServer) return;

  unsigned long nowMs = millis();
  bool skySafariConnected = lx200Client && lx200Client.connected();
  bool skySafariRecentlyActive =
    (lx200WifiLastRxMs && (nowMs - lx200WifiLastRxMs < SKYSAFARI_HTTP_QUIET_MS)) ||
    (lx200WifiLastTxMs && (nowMs - lx200WifiLastTxMs < SKYSAFARI_HTTP_QUIET_MS));

  bool httpSkipWindowExpired = httpLastServiceMs && (nowMs - httpLastServiceMs >= SKYSAFARI_HTTP_MAX_SKIP_MS);
  if ((skySafariConnected || skySafariRecentlyActive) && !httpSkipWindowExpired) {
    httpSkippedForSkySafari++;
    return;
  }

  if (httpLastServiceMs && nowMs - httpLastServiceMs < HTTP_SERVICE_MIN_GAP_MS) return;
  httpLastServiceMs = nowMs;
  httpServicedCount++;
...
#if defined(ESP32)
  // Drop optional protocol sessions before a client burst exhausts lwIP heap.
  if (ESP.getFreeHeap() < NETWORK_LOAD_SHED_HEAP) {
    if (lx200Client) {
      lx200Client.stop();
      lx200WifiClientConnected = false;
    }
    ...
    if (telnetClient) dropTelnetClientForOutputStall();
    return;
  }
#endif
...
void serviceNetworkDuringMountWait() {
  ...
  // Keep mount UART reads timing-safe. Browser and network clients are serviced
  // between mount transactions so multiple open browsers cannot stretch waits.
  serviceLX200DuringMountWait();
  serviceNtpSync();
  delay(1);
  yield();
}

void serviceLX200DuringMountWait() {
  PROFILE_SCOPE(PROFILE_LX200_WAIT_SERVICE);
  if (!bridgeModeHasWifiProtocols()) return;

  dropStaleLX200ClientIfNeeded();

  if (!lx200Client || !lx200Client.connected()) {
    if (lx200WifiClientConnected) {
      LOGI("LX200/SkySafari WiFi client disconnected during mount wait");
      lx200WifiClientConnected = false;
    }

    if (!acceptLX200ClientIfPending("accepted during mount wait")) return;
  }

  if (!lx200Client.available()) return;

  for (uint8_t i = 0; i < 4 && processLX200Stream(lx200Client, LX_SRC_WIFI); i++) {
    yield();
  }
}
...
#if defined(ESP32)
static unsigned long wifiReconnectBackoffMs(uint8_t attempt) {
  unsigned long delayMs = WIFI_RECONNECT_INITIAL_BACKOFF_MS;
  for (uint8_t i = 0; i < attempt && delayMs < WIFI_RECONNECT_MAX_BACKOFF_MS; ++i) {
    delayMs *= 2UL;
  }
...
void serviceWiFiReconnect() {
  ...
  if (!wifiReconnectInitialized) {
    wifiReconnectInitialized = true;
    wifiReconnectWasConnected = connected;
  ...
  if (connected) {
    const bool recovered = !wifiReconnectWasConnected || !staConnected || wifiReconnectAttemptActive;
  ...
  if (wifiReconnectWasConnected || staConnected) {
    wifiReconnectWasConnected = false;
    staConnected = false;
    wifiReconnectAttemptActive = false;
    wifiReconnectAttemptNumber = 0;
    wifiReconnectNextAttemptMs = millis() + 1000UL;
    lastWifiStatus = String("STA disconnected; reconnect pending status=") + wifiStatusCodeText(status);
    Serial.printf("[WIFI] STA disconnected status=%s; reconnect supervisor armed\n",
                  wifiStatusCodeText(status).c_str());
    LOG_WIFI_W("STA disconnected status=%s; reconnect supervisor armed", wifiStatusCodeText(status).c_str());
  }

  if (wifiReconnectAttemptActive) {
    if ((unsigned long)(millis() - wifiReconnectAttemptStartedMs) < WIFI_RECONNECT_ATTEMPT_TIMEOUT_MS) return;
    wifiReconnectAttemptActive = false;
    wifiReconnectNextAttemptMs = millis() + wifiReconnectBackoffMs(wifiReconnectAttemptNumber);
    Serial.printf("[WIFI] STA reconnect attempt timed out; retry in %lums\n",
                  wifiReconnectBackoffMs(wifiReconnectAttemptNumber));
    return;
  }

  // Preserve the intentional AP-only fallback selected during initial setup.
  if (apRunning && WiFi.getMode() == WIFI_AP) return;
  if ((long)(millis() - wifiReconnectNextAttemptMs) < 0) return;
  if (ESP.getFreeHeap() < WIFI_RECONNECT_MIN_HEAP) {
    wifiReconnectNextAttemptMs = millis() + 5000UL;
    lastWifiStatus = "STA reconnect deferred: low heap";
    return;
  }
  ...
  wifiReconnectAttemptActive = true;
  wifiReconnectAttemptStartedMs = millis();
  wifiReconnectAttemptNumber++;
  lastWifiStatus = String("STA reconnect attempt ") + String(wifiReconnectAttemptNumber);
  Serial.printf("[WIFI] STA reconnect attempt %u SSID=%s heap=%u\n",
                (unsigned)wifiReconnectAttemptNumber, staSsid.c_str(), ESP.getFreeHeap());
  LOG_WIFI_I("STA reconnect attempt=%u SSID=%s", (unsigned)wifiReconnectAttemptNumber, staSsid.c_str());
}
#endif
...
String gpioStartupModeText() {
  ...
  s += "\n";
  s += "Selected pin: ";
  s += startupModePinUsed >= 0 ? String("GPIO") + String(startupModePinUsed) : String("none");
  s += "\n";
  s += "Active level: ";
...
#if defined(ESP8266)
  if (bridgeModeHasWebUi() || bridgeModeHasWifiProtocols()) {
    webServer = bridgeModeHasWebUi() ? new ESP8266WebServer(HTTP_WEB_PORT) : nullptr;
    alpacaServer = bridgeModeHasWifiProtocols() ? new ESP8266WebServer(ALPACA_PORT) : nullptr;
    lx200ServerPtr = bridgeModeHasWifiProtocols() ? new WiFiServer(LX200_PORT) : nullptr;
    stellariumServerPtr = bridgeModeHasWifiProtocols() ? new WiFiServer(STELLARIUM_PORT) : nullptr;
  } else {
    webServer = nullptr;
...
#else
  if (bridgeModeHasWebUi() || bridgeModeHasWifiProtocols()) {
    webServer = bridgeModeHasWebUi() ? new AsyncWebCompatServer(HTTP_WEB_PORT) : nullptr;
    alpacaServer = bridgeModeHasWifiProtocols() ? new AsyncWebCompatServer(ALPACA_PORT) : nullptr;
    lx200ServerPtr = bridgeModeHasWifiProtocols() ? new WiFiServer(LX200_PORT) : nullptr;
    stellariumServerPtr = bridgeModeHasWifiProtocols() ? new WiFiServer(STELLARIUM_PORT) : nullptr;
  } else {
    webServer = nullptr;
...
void startFullNetworkListeners() {
  ...
  if (bridgeModeHasWifiProtocols()) discoveryUdp.begin(ALPACA_DISCOVERY_PORT);
...
void handleAlpacaDiscovery() {
  PROFILE_SCOPE(PROFILE_ALPACA_DISCOVERY);
...
void handleLX200Server() {
  PROFILE_SCOPE(PROFILE_LX200_SERVER);
  ...
  if (!lx200Client || !lx200Client.connected()) {
    ...
    acceptLX200ClientIfPending("connected");
    return;
  }
...
void handleStellariumServer() {
  PROFILE_SCOPE(PROFILE_STELLARIUM_SERVER);
...
static uint16_t uiHeight() {
  // Use NAWS/manual height even in Windows compatibility mode; older sizing was incorrect.
...
static void formatBannerColumns(uint8_t index, char *left, size_t leftSize,
  ...
  const char *alpacaState = alpacaHttpRequests ? "RX" : "WAIT";
  ...
  switch (index) {
    ...
    case 3: {
      ...
      const char *system = sampleBannerSystemText();
      snprintf(right, rightSize, "%-*s: %s",
               RIGHT_LABEL_WIDTH, "System", system ? system : "");
      break;
    }
    ...
    case 4:
      ...
      snprintf(right, rightSize, "%-*s: Free:%luK Min:%luK",
               ...
               (unsigned long)(espMinFreeHeapCompat() / 1024UL));
      break;
  }
}
...
static void captureMenuCommand(const char *cmd){
  telnetMenuHeapDiag("capture-before");
  ...
  if (strcmp(cmd,"tasks")==0) {
  ...
  } else if (strcmp(cmd,"menu_goto_radec_help")==0) {
    capture.println("Web Control parity: GOTO RA/Dec");
    capture.println("Use the Telnet prompt command:");
    capture.println("  goto radec <RA hours 0..24) <Dec degrees -90..90>");
    capture.println("The same horizon/altitude safety checks and async GOTO queue are used as the Web UI.");
  } else if (strcmp(cmd,"menu_goto_altaz_help")==0) {
    capture.println("Web Control parity: GOTO Alt/Az");
    capture.println("Use the Telnet prompt command:");
    ...
    capture.println("Telnet exposes the current values through Status -> Time / Location.");
  } else if (strcmp(cmd,"menu_setup_ntp_help")==0) {
    capture.println("Observer Setup parity: Time / NTP");
    capture.println("The Web Setup page is required for NTP enable/server/TZ settings and Sync NTP Now.");
    capture.println("Telnet shows the resulting time source and validity in Status -> Time / Location.");
  } else if (strcmp(cmd,"menu_setup_safety_help")==0) {
    capture.println("Observer Setup parity: Safety limits / rates");
    capture.println("The Web Setup page is required for altitude safety limits and all four nudge rates.");
    capture.println("Telnet exposes polling and nudge controls here; it does not duplicate those Web forms.");
  ...
  telnetMenuHeapDiag("capture-after");
}
...
static void showCommand(Print &out,const char *title,const char *cmd){
  ...
  drawOutput(out,true);
  telnetMenuHeapDiag("command-render-after");
}
...
static bool adjustFocusedNumber(Print &out,int direction) {
  ...
  if(telnetMenuScreen==3 && telnetMenuSelection==1) {
    ...
    savePersistentSettings();
    drawRow(out,1,true);
    drawFooter(out,"Idle mount poll interval changed");
    return true;
  }
...
static bool telnetParseByte(uint8_t ch,uint8_t &app){
  ...
  if(telnetIacState==5){if(ch==TSE){if(telnetSbOption==31&&telnetSbLength>=4){uint16_t w=(telnetSbData[0]<<8)|telnetSbData[1],h=(telnetSbData[2]<<8)|telnetSbData[3];if(w>=20&&w<=300&&h>=8&&h<=120){telnetTermWidth=w;telnetTermHeight=h;telnetNawsReceived=true;if(telnetMenuActive){TelnetOutputGuard socketOut(telnetClient);TelnetCRLFPrint o(socketOut);redrawCurrent(o);o.flush();if(socketOut.stalled())dropTelnetClientForOutputStall();}}}telnetIacState=0;}else if(ch==TIAC){if(telnetSbLength<sizeof(telnetSbData))telnetSbData[telnetSbLength++]=TIAC;telnetIacState=4;}else{telnetSbOption=0;telnetSbLength=0;telnetIacState=0;}return false;}return false;
}
...
static void telnetResetLineEditor();

static void dropTelnetClientForOutputStall() {
  Serial.println("[telnet] output stalled; dropping client");
  if (telnetMenuActive) {
...
static void telnetResetLineEditor() {
  ...
  telnetCommandEscapeState = 0;
  telnetHistoryBrowse = -1;
  telnetHistoryScratch = "";
  telnetRenderedLineLength = 0;
}

static void telnetSaveHistory(const String &line) {
  if (line.length() == 0) return;

  if (telnetHistoryCount > 0) {
    int newest = (int)telnetHistoryWrite - 1;
    if (newest < 0) newest += TELNET_HISTORY_LINES;
    if (telnetHistory[newest] == line) return;
  }

  telnetHistory[telnetHistoryWrite] = line;
  telnetHistoryWrite = (uint8_t)((telnetHistoryWrite + 1) % TELNET_HISTORY_LINES);
  if (telnetHistoryCount < TELNET_HISTORY_LINES) telnetHistoryCount++;
}

static String telnetHistoryAt(uint8_t newestOffset) {
  if (telnetHistoryCount == 0 || newestOffset >= telnetHistoryCount) return "";
  int index = (int)telnetHistoryWrite - 1 - newestOffset;
  while (index < 0) index += TELNET_HISTORY_LINES;
  return telnetHistory[(uint8_t)index];
}

static void telnetRedrawPromptLine(Print &out, const String &line) {
  size_t clearLen = telnetRenderedLineLength;
  if (clearLen < line.length()) clearLen = line.length();
  out.print("\r> ");
  for (size_t i = 0; i < clearLen; i++) out.print(' ');
  out.print("\r> ");
  out.print(line);
  telnetRenderedLineLength = line.length();
}

static bool telnetApplyHistory(int direction, Print &out) {
  if (telnetHistoryCount == 0) return true;

  if (direction < 0) {
    if (telnetHistoryBrowse < 0) {
      telnetHistoryScratch = telnetLine;
      telnetHistoryBrowse = 0;
    } else if (telnetHistoryBrowse < (int8_t)(telnetHistoryCount - 1)) {
      telnetHistoryBrowse++;
    }
    telnetLine = telnetHistoryAt((uint8_t)telnetHistoryBrowse);
    telnetRedrawPromptLine(out, telnetLine);
    return true;
  }

  if (direction > 0) {
    if (telnetHistoryBrowse < 0) return true;
    if (telnetHistoryBrowse > 0) {
      telnetHistoryBrowse--;
      telnetLine = telnetHistoryAt((uint8_t)telnetHistoryBrowse);
    } else {
      telnetHistoryBrowse = -1;
      telnetLine = telnetHistoryScratch;
      telnetHistoryScratch = "";
    }
    telnetRedrawPromptLine(out, telnetLine);
    return true;
...
static bool telnetHandleCommandEscape(uint8_t ch, Print &out) {
  if (telnetCommandEscapeState == 0) {
    if (ch == 27) {
      telnetCommandEscapeState = 1;
      return true;
    }
    return false;
  }

  if (telnetCommandEscapeState == 1) {
    if (ch == '[') {
      telnetCommandEscapeState = 2;
      return true;
    }
    if (ch == 'O') {
      telnetCommandEscapeState = 3;
      return true;
    }
    telnetCommandEscapeState = 0;
    return true;
  ...
  if (telnetCommandEscapeState == 2 || telnetCommandEscapeState == 3) {
    telnetCommandEscapeState = 0;
    if (ch == 'A') return telnetApplyHistory(-1, out);
    if (ch == 'B') return telnetApplyHistory(1, out);
    return true;
  }
...
void serviceTelnetConsole() {
  PROFILE_SCOPE(PROFILE_TELNET_CONSOLE);
#if defined(ESP32)
  ...
  if (!telnetClient || !telnetClient.connected()) {
    ...
    if (nc) {
      ...
      telnetClient.setTimeout(TELNET_SOCKET_WRITE_TIMEOUT_MS);
  ...
  while (telnetClient.available()) {
    ...
    if (telnetAuthenticated && telnetHandleCommandEscape(ch, telnetOut)) {
      continue;
    }
    ...
    if (telnetEndOfLine) {
      ...
      bool commandEndedWithCR = telnetLastWasCR;
      telnetResetLineEditor();
      telnetLastWasCR = commandEndedWithCR;
      line.trim();
      ...
      if (!telnetAuthenticated) {
        ...
        telnetSaveHistory(line);
        telnetExecuteCommand(line, telnetOut);
      }

      // Full-screen modes own the terminal and must not receive a normal
      // command prompt after their start command. A pending confirmation owns
      // the next input line and leaves its [y/N] prompt visible.
      if (telnetClient && telnetClient.connected() &&
          !telnetMenuActive && !telnetMonitorActive && !telnetTasksActive &&
          telnetPendingConfirmCommand.length() == 0) {
        telnetOut.print("> ");
        telnetRenderedLineLength = 0;
      }
      continue;
    ...
    if (ch == 8 || ch == 127) {
      if (telnetLine.length() > 0) {
        ...
        telnetRenderedLineLength = telnetLine.length();
  ...
  if (telnetMonitorActive && millis() - telnetMonitorLastDrawMs >= telnetMonitorRefreshMs) {
    telnetMonitorLastDrawMs = millis();
    telnetDrawMonitor(telnetOut);
  }
  if (telnetTasksActive && millis() - telnetTasksLastDrawMs >= telnetTasksRefreshMs) {
