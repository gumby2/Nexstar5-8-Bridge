...
static unsigned long lastProtocolDemandMarkMs = 0;
static unsigned long mountRecoveryUntilMs = 0;
...
void deferMountPolling(unsigned long delayMs, const char* reason) {
  unsigned long nowMs = millis();
  lastMountPollMs = nowMs;
  nextMountPollDueMs = nowMs + delayMs;
  if ((long)(nextMountPollDueMs - mountRecoveryUntilMs) > 0) mountRecoveryUntilMs = nextMountPollDueMs;
  if (reason) {
    LOG_MOUNT_I("Mount polling deferred for %lu ms: %s", delayMs, reason);
  } else {
    LOG_MOUNT_D("Mount polling deferred for %lu ms", delayMs);
  }
}

void noteMountCommandFailure(const char* reason) {
  consecutiveMountCommandFailures++;
  unsigned long delayMs = consecutiveMountCommandFailures >= 2 ? MOUNT_MULTI_FAIL_RECOVERY_MS : MOUNT_SINGLE_FAIL_RECOVERY_MS;
  if (reason) {
    LOG_MOUNT_W("Mount command recovery pause %lu ms after failure #%u: %s",
                delayMs,
                (unsigned)consecutiveMountCommandFailures,
                reason);
  }
  deferMountPolling(delayMs, "mount command recovery");
}
...
void markPositionDemandThrottled(const char* reason, unsigned long throttleMs) {
  unsigned long nowMs = millis();
  if (lastProtocolDemandMarkMs && nowMs - lastProtocolDemandMarkMs < throttleMs) return;
  lastProtocolDemandMarkMs = nowMs;
  markPositionDemand(reason);
}
...
void resetMountPollFailures() {
  ...
  consecutiveMountCommandFailures = 0;
}
...
void serviceMountPolling() {
  PROFILE_SCOPE(PROFILE_MOUNT_POLL);
