...
void handleLX200Command(const String &rawCmd, uint8_t source) {
  PROFILE_SCOPE(PROFILE_LX200_COMMAND);
  String cmd = rawCmd;
  ...
  if (lx200PollCommand) markPositionDemandThrottled(srcName, 5000);
  if (!lx200PollCommand) {
  ...
  if (cmd == ":D") {
    ...
    // As soon as '@' has arrived, return a bare # so SkySafari exits Stop mode.
