...
public:
  explicit TelnetOutputGuard(WiFiClient &client)
      : client_(client), startedMs_(millis()), stalled_(false) {}
  ...
  size_t write(const uint8_t *data, size_t size) override {
    ...
    if (millis() - startedMs_ >= TELNET_OUTPUT_BUDGET_MS) {
      stalled_ = true;
      return 0;
    }
#if defined(ESP32)
    ...
    const int sent = ::send(socketFd, data, size, MSG_DONTWAIT);
    if (sent <= 0 || (size_t)sent != size || millis() - startedMs_ >= TELNET_OUTPUT_BUDGET_MS) {
      stalled_ = true;
    }
    return sent > 0 ? (size_t)sent : 0;
#else
    size_t written = client_.write(data, size);
    if (written != size || millis() - startedMs_ >= TELNET_OUTPUT_BUDGET_MS) stalled_ = true;
    return written;
#endif
  }
...
static const char *menuLabel(uint8_t screen, uint8_t index) {
  ...
  static const char *setup[] = {"Active mount poll", "Idle mount poll", "Status auto-refresh interval", "Telnet live logging", "", "Back"};
...
static void selectMenu(Print &out){telnetMenuFocus=0;
  ...
  if(telnetMenuScreen==3){if(telnetMenuSelection<=2)adjustFocusedNumber(out,+1);else if(telnetMenuSelection==3)toggleFocusedCheckbox(out);else if(telnetMenuSelection==4){if(bridgeMode==BRIDGE_MODE_WIFI_FULL)showConfirm(out,"Bluetooth mode","Save BT + WiFi Telnet mode and reboot?","mode bt");else showConfirm(out,"Full WiFi mode","Save Full WiFi mode and reboot?","mode wifi");}else backMenu(out);return;}
