...
void deferMountPolling(unsigned long delayMs, const char* reason = nullptr);
void noteMountCommandFailure(const char* reason = nullptr);

void markPositionDemand(const char* reason = nullptr);
void markPositionDemandThrottled(const char* reason = nullptr, unsigned long throttleMs = 5000);
